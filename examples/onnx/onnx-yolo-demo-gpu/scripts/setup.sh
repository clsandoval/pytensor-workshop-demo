...
echo "=========================================="
echo "YOLO11n GPU Training Environment Setup"
echo "=========================================="
echo ""
...
cd examples/onnx/onnx-yolo-demo-gpu/
...
echo "Installing YOLO demo dependencies (with CUDA-enabled JAX)..."
uv pip install -e .
...
echo "Verifying JAX CUDA installation..."
JAX_DEVICES=$(uv run python -c "import jax; print(jax.devices())" 2>&1)
if echo "$JAX_DEVICES" | grep -q "CudaDevice"; then
    echo "✓ JAX CUDA successfully configured"
    echo "  Devices: $JAX_DEVICES"
elif echo "$JAX_DEVICES" | grep -q "GpuDevice"; then
    echo "✓ JAX GPU successfully configured"
    echo "  Devices: $JAX_DEVICES"
else
    echo "⚠ WARNING: JAX not using GPU!"
    echo "  Devices: $JAX_DEVICES"
    echo "  Training will be VERY slow without GPU."
    echo "  Check CUDA installation and try reinstalling JAX:"
    echo "    uv pip uninstall jax jaxlib"
    echo "    uv pip install --upgrade 'jax[cuda]>=0.4.20'"
fi
echo ""
