"""YOLO11n model with JAX-compatible upsampling fix."""
...
class YOLO11nHeadFixed:
    """
    YOLO11n detection head with JAX-compatible upsampling.
    ...
    def __call__(self, p3, p4, p5):
        ...
        # FPN path (top-down)
        # P5 → P4
        p5_up = self._upsample_jax_compatible(p5, scale=2)
        p4_fused = pt.concatenate([p5_up, p4], axis=1)
        p4_out = self.c3k2_p4(p4_fused)

        # P4 → P3
        p4_up = self._upsample_jax_compatible(p4_out, scale=2)
    ...
    def _upsample_jax_compatible(self, x, scale=2):
        """
        JAX-compatible upsampling using repeat operation.

        This avoids dynamic shape computations that create JAX tracers.
        """
        # x: (batch, C, H, W)
        # Use repeat instead of reshape with dynamic shapes

        # Method 1: Use repeat along each spatial dimension
        # This is JAX-compatible because repeat doesn't require computing new shapes

        # Repeat along height dimension
        x_repeat_h = repeat(x, scale, axis=2)
        # Repeat along width dimension
        x_upsampled = repeat(x_repeat_h, scale, axis=3)
...
class YOLO11nFixed:
    """
    Complete YOLO11n model with JAX-compatible operations.

    Combines backbone and fixed head for end-to-end object detection.
    """
...
def build_yolo11n_jax_fixed(num_classes=2, input_size=320):
    """
    Build YOLO11n model with JAX-compatible operations.
    ...
    # Model
    model = YOLO11nFixed(num_classes=num_classes, input_size=input_size)
